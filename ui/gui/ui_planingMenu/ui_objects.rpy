screen screen_objects(player):
    ...
    if renpy.get_screen('screen_operate') and not renpy.get_screen("screen_phone_camera_show") and not HotelBuff.has(player) and not BookstoreBuff.has(player) and not CafeBuff.has(player):
            ...
            imagebutton idle showcactus.sprite():
                ...
                if type(showcactus) != DeadCactus:
                    if persistent.actionquickly:
                        action Hide("info"),Function(showcactus.water, player)
                    else:
        ...
        if showcactus and player.onVacation and player.experience=='wri' and p.times <= 9:
            imagebutton idle 'gui/object/shadow.png':
                at disso
                xcenter 0.9
                ycenter 0.65
        ...
        if ToyDuck.has(player) and player.onVacation and player.experience=='wri' and p.times <= 9:
