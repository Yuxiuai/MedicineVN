screen screen_map(player):
    ...
    frame:
        ...
        imagebutton auto "imagemap/ShoppingStreet_%s.png":
            focus_mask True
            action [Hide("info"), Show(screen="screen_shopstreet_map", player=player)]
            hover_sound audio.cursor
    ...
    hbox:
        ...
        imagebutton auto "gui/menu/home_%s.png":
            at trans_toLeft()
            action Hide("screen_map", transition=dissolve)
            unhovered Hide("info")
            hovered Show(screen="info", i=_('关闭地图'), a=_('该继续安排日程了。'))
            activate_sound audio.cursor

    key 'K_ESCAPE' action Hide("screen_map", transition=dissolve)




screen screen_shopstreet_map(player):
    #tag gamegui
    zorder 900
    frame:
        at map_appear
        ...
        imagebutton auto "imagemap/shopstreet/hotel_%s.png":
            focus_mask True
            #action [Hide("info"), Show(screen="info_use",text='去这里',act=[Hide("screen_explore_map"),Hide("info_use"),Return(),Jump("explore_bookshop")], pp=renpy.get_mouse_pos(),i=info1+info11, a=ad1)]
            action NullAction()
            hover_sound audio.cursor
            hovered Show("info",i=info1+info11, a=ad1)
            unhovered Hide("info")
        ...
        $ad3 = '不是我不喜欢打游戏，只是一激动，一失败，或者只是玩的久一些，我的头疼就会把我的脑壳炸开来。' if 1 in p.visitedStore else '？？？？？？？？？？？？'
        ...
        imagebutton auto "imagemap/shopstreet/sportshop_%s.png":
            focus_mask True
            action [Hide("info"), Show(screen="info_use", pp=renpy.get_mouse_pos(),i=info9+info9, a=ad9)]
            hover_sound audio.cursor
            hovered Show("info",i=info9+info9, a=ad9)
            unhovered Hide("info")
