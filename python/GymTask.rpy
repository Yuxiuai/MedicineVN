    ...
    class GymTask:
        ...
        def executeTask(cls, player):
            if not GymTicket.has(player):
                player.money -= r2(0.2*player.price)
            ...
            for i in range(len(player.gymplan)):
                ...
                renpy.say(None, "准备进行%s……"%player.gymplan[i].name)
                renpy.show_screen(_screen_name="Task_processing_screen",player=p)
                renpy.say(None, "{cps=3}。。。。。。")
                ...
                if Injured.has(player):
                    ...
                    temp = rd(1,4)
                    if temp == 1:
                        renpy.say(None, "呼，算是比较成功地做完了……")
                    elif temp == 2:
                        renpy.say(None, "幸好没受伤……看来我得稍微小心点。")
                    elif temp == 3:
                        renpy.say(None, "刚才坐在旁边的狼兽人穿着白袜哦，不会是男同吧……")
                    else:
                        renpy.say(None, "好累啊……好想休息……")
