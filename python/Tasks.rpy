init python early:
    ...
    class DefaultWork(WorkTask):
        ...
        def excePerf(cls, player):
            ...
            Notice.add(_('升高了%s点工作能力。') % (int(g * 100)))
    ...
    class LoafingWork(WorkTask):
        ...
        def executeAnotherTask(cls, player, doWith, perf):
            ...
            if doWith == 'sleep':
                ...
                Notice.add(_('在工作的间隙中尝试小睡，恢复了%s点精神状态。') % rec)
                player.mental += rec
                ...
                Notice.add(_('在工作的间隙中尝试做些杂活，恢复了%s点精神状态。') % rec)
                if p == 2:
                    player.physical += 0.01
                    Notice.add(_('流了些汗，提升了1点身体素质。'))
                else:
                    PhysRezB.add(player)
                    Notice.add(_('流了些汗，获得了状态：良好的运动。'))
            elif doWith == 'wri' and player.canRead >= 0:
                rec = r2(10 * cls.getRecoScale(player) * player.sleepRecovery * p)
                Notice.add(_('在工作的间隙中看了一会网络小说，恢复了%s点精神状态，获得了1层灵感。') % rec)
                Inspiration.add(player)
                if p == 2:
                    player.writing += 0.01
                    Notice.add(_('对写作有了更多的想法，提升了1点写作技巧。'))
                else:
                    Notice.add(_('对写作有了更多的想法，获得了状态：精神的释放。'))
                    MentRezA.add(player)
                ...
                if book is not None:
                    ...
                    if MeetingReward5.has(player):
                        pro = 2
                    if book.progress == 0:
                        Notice.add(_('从头开始阅读书本：%s……') % type(book).name)
                        book.readBook(player, pro)
                        
                    elif book.progress == 1:
                        Notice.add(_('从上次看过的位置继续阅读书本：%s……') % type(book).name)
                        book.readBook(player, pro)
                        
                    if book.progress == 1:
                        Notice.add(_('完成了一半的进度！'))
                    elif book.progress == 2:
                        Notice.add(_('完成了整本书的阅读！'))
            else:
                renpy.say(None, _('任务参数错误！'))
    ...
    class DefaultSport(SportTask):
        ...
        def excePerf(cls, player):
            ...
            player.physical += g
            Notice.add(_('恢复了%s点精神状态。') % reco)
            Notice.add(_('额外恢复了%s点精神状态。') % exReco)
    ...
    class FreewheelingWriting(WriteTask):
        ...
        def checkAvailable(cls, player, day, time):
            ...
            if list(filter(lambda x: type(x) in (MentProb,MentPun,Erection,Sadness,Pleasure), player.effects))==[]:
                return _('你不知道写什么。')
            return True

        @classmethod
        def executeTask(cls, player):
            l = list(filter(lambda x: type(x) in (MentProb,MentPun,Erection,Sadness,Pleasure), player.effects))
            s = 0
            for i in (MentProb,MentPun,Erection,Sadness,Pleasure):
                if i.has(player):
                    s += i.getstack(player)
    ...
    class ReadingBook(WriteTask):
        ...
        def checkAvailable(cls, player, day, time):
            if not player.onVacation and time != 2:
                return _('现在是正常上班时间！')
            if list(filter(lambda x: x.kind == '书本' and type(x).__name__ not in player.itemcd, player.items)) == []:
    ...
    class Sleep(RestTask):
        ...
        def executeTask(cls, player):
            ...
            if not player.s6 and player.today in (6, 7) and player.week >= 2:
                if Inspiration.has(player):
                    if Inspiration.getstack(player) >= 5:
                        renpy.jump("solitus_route_6")
            if not player.s9 and Erection.has(player) and player.week >= 2:
    ...
    class ComputerGaming(RestTask):
        ...
        def afterTaskResult(cls, player):
            if PhysProb.has(player):
                PhysProb.get(player).timeUpdate(player, ra(player, 1, 2))
            
            if MentProb.has(player):
                MentProb.get(player).timeUpdate(player, ra(player, 1, 2))
    ...
    class SocialMedia(RestTask):
        ...
        unlocked = False
        info = _('基础恢复：15\n该日程不受专注度影响。')
        ad = _('和网上的怪胎们聊聊天，或者在莓博上翻翻有没有好看的画作。')
       
        @classmethod
        def unlockCond(cls, player):
            return _('当前版本尚未开放此日程。')
    ...
    class AcolasTask1(Task):
        ...
        def executeTask(cls, player):
            ...
            player.mental -= r2(cons)
            Notice.add(_('消耗了%s点精神状态。') % r2(cons))
            Notice.add(_('完成了%s%s的进度。') % (r2(a), '%'))
            if 100 > item.progress:
                Notice.add(_('还差%s%s。') % (r2(100-item.progress), '%'))
            if item.progress > 100:
                item.progress = 100.0
            Notice.add(_('提升了5点严重程度。'))
            PhysProb.add(player, 3)

            
            player.updateAfterTask(cls)
            if AcolasItem3.has(player) and player.times == 12 and (player.today == 4 or player.today == 5 and int(player.st()[0]) < 6) and player.aco_p == 8:
                renpy.jump("AcolasTask1_loop")
            elif AcolasItem4.has(p) and player.times == 12:
                aa = 0.01 * int(a)
                if aa > 0:
                    player.severity += aa
                    Notice.add(_('提升了%s点严重程度。') % int(a))
