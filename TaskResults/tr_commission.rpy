screen screen_tr_commission(player):
    ...
    $ items = list(filter(lambda x: type(x) == UnfinishedCommission , player.items))
        ...
        frame:
            ...
            vbox:
                frame:
                    background None
                    yalign 0.001
                    textbutton _('{size=+10}选择要写作的委托{/size}'):
                        text_style "white"
                        xoffset -5
                        yoffset -5
                        action NullAction()
                ...
                frame:
                    ...
                    frame:
                        ...
                        if ite.comm.checkWritable(player)!=True:
                            ...
                            textbutton _('{color=#b3b3b3}') +ite_name+_('{/color}') text_style "white":
                                action [Function(showNotice, [error_info]),NullAction()]
                                hovered [Show(screen="info", t=ite_name, i=ite_main+ite_suf, a=ite.ad)]
                                unhovered Hide("info")
                                background Frame("gui/style/grey_[prefix_]background.png", Borders(0, 0, 0, 0), tile=gui.frame_tile)
                                activate_sound audio.cursor
                                xfill True
                            ...
                            textbutton ite_name text_style "white":
                                action [Hide("info"),Show(screen="info_confirm",text=_('选择'), act=[Function(player.rtn, ite), Return()], pp=renpy.get_mouse_pos(), t=ite_name, i=ite_main+ite_suf, a=ite.ad)]
                                hovered [Show(screen="info", t=ite_name, i=ite_main+ite_suf, a=ite.ad)]
                                unhovered Hide("info")
                                background Frame("gui/style/grey_[prefix_]background.png", Borders(0, 0, 0, 0), tile=gui.frame_tile)
                                activate_sound audio.cursor
...
screen screen_tr_commission_inputs(player):
    ...
    key 'K_KP_ENTER' action SetScreenVariable("inputwords", inputwords+_('\n        '))
    key 'K_RETURN' action SetScreenVariable("inputwords", inputwords+_('\n        '))
